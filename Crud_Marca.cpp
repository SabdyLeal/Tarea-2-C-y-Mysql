...
using namespace std;
	class clempresa{
		...
		char* conectar(conectar,"localhost","root", "2021", "db_empresa.marca", 3306, NULL,0 ){
			
				if(!(obj=mysql_init(0)))
					{
						return "Error Create Objeto";
					}
					if(!mysql_real_connect("localhost","root", "42895097", "db_empresa.marca", 3306, NULL,0 )
					{
						return (char*)mysql_error(obj);
					}else
					{
						return "-------------------> Base de Datos en ejecución <-------------------";
					}
		}
	...
	class clempresa{
		public:
			...
			void insertar(struct stempresa ){
				try{
				conexion.conectar("localhost","root","","db_empresa.marca");
				char *consulta;
				char sentencia[]="INSERT INTO  db_empresa.marca(ID,NOMBRE,PATERNO) VALUES (\'%s\',\'%s\',\'%s\')";
			...
			void modificar(struct stempresa){
				try{
				conexion.conectar("localhost","root","","db_empresa.marca";
				char *consulta;
			...
			void eliminar(struct stempresa){
				try{
				conexion.conectar("localhost","root","","db_empresa.marca");
				char *consulta;
			...
			void mostrar(){
				   ...
				   if(!mysql_real_connect(objDatos,"localhost", "root", "", "db_empresa.marca", 3306, NULL, 0)) {
				      cout<<"Error BD"<<MYSQL_PORT<<" Error: "<<mysql_error(objDatos)<<endl;
				      mysql_close(objDatos);
				      rewind(stdin);
				      getchar();
				   }
				   if(mysql_select_db(objDatos, "db_empresa.marca")) {
