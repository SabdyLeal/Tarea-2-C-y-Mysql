...
using namespace std;
	class conne{
		...
		char* conectar(conectar,"localhost","root", "2021", "db_empresa.producto", 3306, NULL,0 ){
			
				if(!(obj=mysql_init(0)))
					{
						return "Error Create Objeto";
					}
					if(!mysql_real_connect("localhost","root", "42895097", "db_empresa.producto", 3306, NULL,0 )
					{
	...
	class clproductros{
		public:
			...
			void insertar(struct stempresa ){
				try{
				conexion.conectar("localhost","root","","db_empresa.producto");
				char *consulta;
			...
			void mostrar(){
				   ...
				   if(!mysql_real_connect(objDatos,"localhost", "root", "", "db_empresa.producto", 3306, NULL, 0)) {
